...
@php
  ...
  // Fallback fix for header values
  $weekNo   = $doc->week_no   ?? ($detail->week_no   ?? '');
  $formNo   = $doc->form_no   ?? ($detail->form_no   ?? '');
  ...
  $sigBase64 = null;
  if ($doc->approved_at) {
      // ?? Change this path to the actual stored signature file if per-user
      // Example: $doc->approvedByUser->signature_path
