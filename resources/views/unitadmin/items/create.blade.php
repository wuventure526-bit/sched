...
<div class="card mx-3 mb-3">
    <div class="card-header pb-3">
        ...
        <p class="text-sm mb-0">{{ auth()->user()->seesAllUnits() ? 'Add a new item and choose which unit it belongs to.' : 'Easily expand your inventory by adding a new item to your unit.' }}</p>
    </div>
    <div class="card-body pt-0">
        <form method="POST" action="{{ route('items.store') }}" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                    {{-- Administrators belong to no unit, so they choose the one this item is filed under. --}}
                    @if ($units->isNotEmpty())
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                            <option value="available" {{ old('status', 'available') === 'available' ? 'selected' : '' }}>In service</option>
                            <option value="not available" {{ old('status') === 'not available' ? 'selected' : '' }}>Out of service</option>
                        </select>
