...
<div class="card mx-3 mb-4">
  ...
  <div class="card-body px-0 pt-0 pb-2">
    ...
    <div class="btn-group mb-2">
      ...
      <a class="px-4 py-2 mb-0 btn btn-white text-normal {{ $status === 'pending' ? 'tab-active' : '' }}"
         href="{{ route('bookings.index', ['status' => 'pending']) }}">Pending</a>

      <a class="px-4 py-2 mb-0 btn btn-white text-normal {{ $status === 'approved' ? 'tab-active' : '' }}"
         href="{{ route('bookings.index', ['status' => 'approved']) }}">Approved</a>

      <a class="px-4 py-2 mb-0 btn btn-white text-normal {{ $status === 'rejected' ? 'tab-active' : '' }}"
         href="{{ route('bookings.index', ['status' => 'rejected']) }}">Rejected</a>

      <a class="px-4 py-2 mb-0 btn btn-white text-normal {{ $status === 'cancelled' ? 'tab-active' : '' }}"
         href="{{ route('bookings.index', ['status' => 'cancelled']) }}">Cancelled</a>

      <a class="px-4 py-2 mb-0 btn btn-white text-normal {{ $status === 'expired' ? 'tab-active' : '' }}"
         href="{{ route('bookings.index', ['status' => 'expired']) }}">Expired</a>
    </div>
      ...
      <table class="table align-items-center mb-0">
        ...
        <thead>
          <tr>
            ...
            <th class="text-xxs font-weight-bolder px-2">Memo</th>
            ...
            @if ($status === 'approved' || $status === null)
              ...
              <th class="text-xxs font-weight-bolder px-2">Item Status</th>
            @endif
        ...
        <tbody>
          @foreach ($bookings as $booking)
            <tr>
              ...
              <td>
                <p class="text-xs font-weight-bold mb-0">{{ $booking->jo_number }}</p>
              </td>
              ...
              <td class="align-middle">
                ...
                @elseif ($booking->status === 'cancelled')
                  <span class="badge bg-info badge-sm">Cancelled</span>
                @else
                  <span class="badge bg-secondary badge-sm">{{ ucfirst($booking->status) }}</span>
                @endif
              </td>
              ...
              @if ($status === 'approved' || $status === null)
                <td class="align-middle">
                  <span class="text-xs font-weight-bold">
                    {{ optional($booking->usage)->due_date ?? '—' }}
                  </span>
                </td>

                <td class="align-middle">
                  @if ($booking->usage && $booking->usage->status === 'returned')
                    <span class="badge bg-success badge-sm">Returned</span>
                  @else
                    <span class="badge bg-warning badge-sm">In Use</span>
                  @endif
                </td>
              @endif
              ...
              <td>
                <div class="d-flex align-items-center">
                  <a href="{{ route('bookings.show', $booking->id) }}" class="me-2">
                    <button class="btn btn-action btn-info mb-0" title="View booking">
                      <i class="fas fa-eye"></i>
                    </button>
                  </a>

                  @if ($booking->status === 'approved')
                    <a href="{{ route('bookings.approval', $booking->id) }}" target="_blank" class="me-2">
                      <button class="btn btn-action btn-success mb-0" title="Print approval">
                  ...
                  @if ($booking->status === 'pending')
                    <a href="{{ route('bookings.cancel', $booking->id) }}"
                      ...
                      <button class="btn btn-action btn-danger mb-0" title="Cancel booking">
